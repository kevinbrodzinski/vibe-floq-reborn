-- Optimized Schema Migration - Production Ready (Final Fix)
...
-- 4. Safe index optimization - Drop existing indexes and create new ones without dynamic predicates
DROP INDEX IF EXISTS idx_floqs_location_visibility_time;
DROP INDEX IF EXISTS idx_floqs_location;
DROP INDEX IF EXISTS idx_floqs_ends_at_public;
DROP INDEX IF EXISTS idx_flock_relationships_strength;
DROP INDEX IF EXISTS idx_flock_auto_suggestions_user_type;
DROP INDEX IF EXISTS idx_floqs_activity_location;

-- Create optimized indexes (no dynamic predicates)
CREATE INDEX IF NOT EXISTS idx_floqs_location_gist 
ON public.floqs USING GIST (location);

CREATE INDEX IF NOT EXISTS idx_floqs_public_ends_at 
ON public.floqs (ends_at, visibility)
WHERE visibility = 'public';

CREATE INDEX IF NOT EXISTS idx_flock_relationships_strength_composite
ON public.flock_relationships (relationship_strength DESC, last_interaction_at DESC);

CREATE INDEX IF NOT EXISTS idx_flock_suggestions_user_expires
ON public.flock_auto_suggestions (user_id, expires_at DESC, status);

CREATE INDEX IF NOT EXISTS idx_floqs_activity_score_desc
ON public.floqs (activity_score DESC, starts_at, visibility)
WHERE visibility = 'public';

-- 5. Update suggestion functions with SECURITY INVOKER for RLS safety
...
-- 6. Grant permissions to service_role only for core functions
