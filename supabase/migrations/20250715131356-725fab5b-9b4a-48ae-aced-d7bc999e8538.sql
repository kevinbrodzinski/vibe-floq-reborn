...
-- Create partial unique index for pending requests (not a constraint)
...
-- Add trigger-based validation for future expiry (not CHECK constraint)
CREATE OR REPLACE FUNCTION validate_pin_expiry()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.expires_at <= now() THEN
    RAISE EXCEPTION 'Pin expiry must be in the future';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER pin_expiry_validation
  BEFORE INSERT OR UPDATE ON public.shared_location_pins
  FOR EACH ROW EXECUTE FUNCTION validate_pin_expiry();
