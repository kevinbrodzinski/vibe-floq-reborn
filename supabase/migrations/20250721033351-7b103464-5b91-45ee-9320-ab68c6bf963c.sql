...
-- 4. Creator & participants can update --------------------------------
create policy "floq_plans_creator_participant_update"
  on public.floq_plans
  for update
  using (
    creator_id = auth.uid()
...
-- 5. Creator & participants can delete --------------------------------
create policy "floq_plans_creator_participant_delete"
  on public.floq_plans
  for delete
  using (
    creator_id = auth.uid()
...
as $$
begin
  insert into public.plan_participants(plan_id, user_id, joined_at)
