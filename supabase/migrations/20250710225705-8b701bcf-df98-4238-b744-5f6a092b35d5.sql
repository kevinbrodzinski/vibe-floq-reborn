...
-- Add unique constraint to prevent duplicate requests (check if exists first)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'friend_requests_user_friend_unique'
  ) THEN
    ALTER TABLE friend_requests 
    ADD CONSTRAINT friend_requests_user_friend_unique 
