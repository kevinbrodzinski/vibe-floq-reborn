-- Phase 3.1: Production-Safe Username Schema Migration (Optimized - Fixed CHECK constraint)
...
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_username_not_reserved_citext_check;
...
-- 8. Recreate the v_friends_with_profile view
...
-- 9. Create demo schema and tables
...
-- 10. Add constraints to demo schema (table constraints for consistency)
ALTER TABLE demo.profiles 
ADD CONSTRAINT demo_profiles_username_unique UNIQUE (username);

-- 11. Optimized safety triggers (only run when username actually changes)
...
-- 12. Reserved username protection via triggers (more flexible than CHECK constraints)
CREATE OR REPLACE FUNCTION prevent_reserved_usernames()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER profiles_prevent_reserved_usernames
  BEFORE INSERT OR UPDATE OF username ON public.profiles
  FOR EACH ROW EXECUTE FUNCTION prevent_reserved_usernames();

CREATE OR REPLACE FUNCTION prevent_demo_reserved_usernames()
RETURNS TRIGGER AS $$
BEGIN
  IF EXISTS (SELECT 1 FROM demo.reserved_usernames WHERE name = NEW.username) THEN
    RAISE EXCEPTION 'Username "%" is reserved and cannot be used', NEW.username;
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER demo_profiles_prevent_reserved_usernames
  BEFORE INSERT OR UPDATE OF username ON demo.profiles
  FOR EACH ROW EXECUTE FUNCTION prevent_demo_reserved_usernames();
...
-- Populate main reserved usernames if empty
