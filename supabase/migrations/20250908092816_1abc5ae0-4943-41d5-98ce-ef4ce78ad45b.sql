...
--    Simplified RSVP logic to work with existing data
-- ======================================================================
...
AS $$
  ...
  -- RSVP weights (simplified - give small bonus if both have same status)
  pair_rsvp AS (
    SELECT
      p.a, p.b,
      CASE WHEN ra.rsvp_status IS NOT NULL AND rb.rsvp_status IS NOT NULL 
           AND ra.rsvp_status = rb.rsvp_status THEN 0.20
           ELSE 0.10 END AS w_rsvp
    FROM pairs p
  ...
  prox AS (
    SELECT
      ...
      CASE
        ...
        THEN 0.05  -- Small default bonus for having data
        ELSE (
          0.25 * (
            1 - LEAST(1.0,
              ABS(
  ...
  reason_list AS (
    SELECT ARRAY_REMOVE(ARRAY[
      'RSVP alignment considered',
      (SELECT CASE WHEN EXISTS (SELECT 1 FROM likes) THEN 'Venue preferences grouped' END),
      (SELECT CASE WHEN EXISTS (SELECT 1 FROM coplans) THEN 'Historical co-plans considered' END),
      (SELECT CASE WHEN EXISTS (SELECT 1 FROM home_centers) THEN 'Location patterns analyzed' END),
      (SELECT CASE WHEN EXISTS (SELECT 1 FROM hour_hist) THEN 'Activity timing considered' END)
    ]::text[], NULL) AS reasons
  )
