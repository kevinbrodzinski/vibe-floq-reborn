...
-- (Safe in Supabase; no-op if already installed)
CREATE EXTENSION IF NOT EXISTS postgis;

-- ============================================================
-- A) Minimal audit table: group_receipts (append-only, RLS on)
-- ============================================================
CREATE TABLE IF NOT EXISTS public.group_receipts (
  ...
  event_type  text NOT NULL CHECK (
                event_type IN (
                  ...
                  'RELAX_APPLIED','RADIUS_EXPANDED','SWITCH_SUGGESTED',
                  'SWITCH_ACCEPTED','SWITCH_REJECTED'
                )
              ),
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname='public' AND tablename='group_receipts' AND polname='gr_select_participants'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='group_receipts' AND polname='gr_insert_participants'
  ) THEN
...
-- ============================================================
-- B) In-DB candidate ranker: fn_ai_suggest_venues (numeric price level)
--    No dollar-signs; uses price_level integer (1..4)
-- ============================================================
...
AS $fn$
  WITH params AS (
    SELECT
      p_lat  AS lat,
      p_lng  AS lng,
      GREATEST(100, p_radius_m) AS radius_m,
  ...
  base AS (
    SELECT
      ...
      -- category overlap (if categories provided)
      CASE
        WHEN (SELECT categories FROM params) IS NULL
          OR array_length((SELECT categories FROM params),1) IS NULL
          THEN true
      ...
      -- open-now (UTC); handles overnight bands
      EXISTS (
        SELECT 1
