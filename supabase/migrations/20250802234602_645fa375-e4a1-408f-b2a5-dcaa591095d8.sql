-- Fix venue schema by dropping and recreating functions
-- Migration: 20250802_fix_venue_schema_final

-- Step 1: Add only missing non-generated columns
DO $$
BEGIN
...
-- Step 4: Drop old unique constraint safely and add new one
...
-- Step 5: Add new unique constraint on (source, external_id)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Step 7: Drop existing functions first
DROP FUNCTION IF EXISTS public.get_cluster_venues(double precision,double precision,double precision,double precision,integer,text,integer);
DROP FUNCTION IF EXISTS public.get_venues_in_bbox(double precision,double precision,double precision,double precision);

-- Step 8: Recreate functions with new return types
CREATE FUNCTION public.get_cluster_venues(
...
-- Step 9: Ensure RLS policy exists
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Step 10: Update table statistics for query planning
