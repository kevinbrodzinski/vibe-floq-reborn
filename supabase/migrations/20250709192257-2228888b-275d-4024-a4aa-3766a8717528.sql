-- Enhanced Avatar Storage Migration v2 (Production-Ready) - FINAL FIX
...
-- Automatic variant sync trigger function (completely fixed type handling)
...
BEGIN
  ...
  IF TG_OP = 'DELETE' THEN
    BEGIN
      target_user_id := OLD.owner::uuid;
      DELETE FROM public.avatar_variants WHERE user_id = target_user_id;
    EXCEPTION WHEN invalid_text_representation THEN
      -- Skip if owner is not a valid UUID
      NULL;
    END;
    RETURN OLD;
  END IF;
...
-- Optimized index for avatar lookups
