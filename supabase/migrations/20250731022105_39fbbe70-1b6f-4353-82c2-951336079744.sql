/* =====================================================================
   External Place Feeds – final integration
   --------------------------------------------------------------------- */
...
BEGIN
  ...
  visits AS (
    ...
    SELECT u.user_id, v.id, now(), 25
    FROM   unraveled u
    JOIN   venues     v USING (provider, provider_id)
    ON CONFLICT DO NOTHING
    RETURNING 1
  )
  SELECT COUNT(*) INTO done FROM visits;
...
SELECT '*/2 * * * *', $$SELECT integrations.normalise_place_feed();$$, 'normalise_place_feed'
WHERE NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'normalise_place_feed');
