...
begin
  ...
  if not exists (select 1 from pg_policies where policyname = 'plans_select_visible') then
  ...
  if not exists (select 1 from pg_policies where policyname = 'plans_insert_creator') then
    create policy plans_insert_creator on public.floq_plans
      for insert with check (creator_id = auth.uid());
  end if;

  if not exists (select 1 from pg_policies where policyname = 'plans_update_creator_draft') then
  ...
  if not exists (select 1 from pg_policies where policyname = 'pp_select_visible') then
  ...
  if not exists (select 1 from pg_policies where policyname = 'pp_insert_creator') then
  ...
  if not exists (select 1 from pg_policies where policyname = 'drafts_rw_creator') then
  ...
  if not exists (select 1 from pg_policies where policyname = 'activities_select_visible') then
  ...
  if not exists (select 1 from pg_policies where policyname = 'ai_summaries_select_visible') then
