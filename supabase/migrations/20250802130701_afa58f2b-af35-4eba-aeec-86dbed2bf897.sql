-- Drop and recreate rank_nearby_people to include demo users and provide synthetic IDs
...
AS $function$
  ...
  SELECT
    ...
    CASE 
      WHEN vn.profile_id IS NULL THEN 'demo-' || encode(digest(vn.vibe || vn.location::text || vn.updated_at::text, 'sha256'), 'hex')::text
