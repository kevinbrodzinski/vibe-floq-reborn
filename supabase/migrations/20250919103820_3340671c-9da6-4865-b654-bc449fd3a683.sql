-- Field rallies RPC with PostGIS index and Wings Poll tables
create index if not exists idx_rallies_center_gist
  on public.rallies using gist ((center::geography));

-- RPC: rallies_field_nearby for distance-filtered field rallies
...
returns table(
  ...
  my_friends as (
    -- use low/high canonical ordering for O(1) friend checks
    select case
  ...
  select r.id, r.creator_id, r.created_at, r.expires_at, r.status,
         r.venue_id, r.scope, r.note
...
-- Wings Poll tables if not exists
...
for insert with check (
  ...
  exists(select 1 from public.floq_wings_events e
         join public.floq_participants fp
           on fp.floq_id = e.floq_id and fp.profile_id = auth.uid()
         where e.id = floq_wings_votes.event_id)
);

drop policy if exists "wings_vote_select" on public.floq_wings_votes;
create policy "wings_vote_select" on public.floq_wings_votes
for select using (
  exists(select 1 from public.floq_wings_events e
         join public.floq_participants fp
           on fp.floq_id = e.floq_id and fp.profile_id = auth.uid()
...
drop policy if exists "wings_vote_delete" on public.floq_wings_votes;
create policy "wings_vote_delete" on public.floq_wings_votes
for delete using (profile_id = auth.uid());

-- Wings poll tally RPC
...
returns table(option_idx int, votes int) language sql stable as $$
  select option_idx, count(*)::int as votes
