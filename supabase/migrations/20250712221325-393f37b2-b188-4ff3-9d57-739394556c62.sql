-- Phase 3.1: Production-Safe Username Schema Migration (Clean Final)
...
-- 4. Drop ALL existing constraints and indexes that might conflict
...
-- 7. Create NEW unique indexes with different names
CREATE UNIQUE INDEX profiles_username_citext_unique ON public.profiles (username);
CREATE UNIQUE INDEX reserved_usernames_citext_unique ON public.reserved_usernames (name);
...
DROP INDEX IF EXISTS demo_profiles_username_unique;
DROP INDEX IF EXISTS demo_reserved_usernames_unique;
CREATE UNIQUE INDEX demo_profiles_username_citext_unique ON demo.profiles (username);
CREATE UNIQUE INDEX demo_reserved_usernames_citext_unique ON demo.reserved_usernames (name);

-- 12. Add constraint to demo schema
ALTER TABLE demo.profiles DROP CONSTRAINT IF EXISTS demo_profiles_username_not_reserved_check;
...
DROP INDEX IF EXISTS profiles_username_trgm;
DROP INDEX IF EXISTS demo_profiles_username_trgm;
...
DROP POLICY IF EXISTS "Demo profiles are publicly readable" ON demo.profiles;
DROP POLICY IF EXISTS "Demo reserved usernames are readable" ON demo.reserved_usernames;
