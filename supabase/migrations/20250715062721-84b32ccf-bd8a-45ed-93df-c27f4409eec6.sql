...
agg AS (
  SELECT
    ...
    COUNT(*) AS total,
    jsonb_object_agg(vibe_tag, cnt ORDER BY vibe_tag) AS vibe_counts
  FROM vibe_counts_sub
  GROUP BY gh6
  HAVING COUNT(*) >= 3
)
SELECT
...
BEGIN
  ...
  ELSE
    ...
    SELECT 
      lf.gh6,
      ST_PointOnSurface(ST_Collect(vcs.geom)) as centroid,
      COUNT(*)::bigint as total,
      jsonb_object_agg(vcs.vibe_tag, vcs.cnt ORDER BY vcs.vibe_tag) as vibe_counts
    FROM vibe_counts_sub vcs
    JOIN live_filtered lf ON lf.gh6 = vcs.gh6
    GROUP BY lf.gh6
    HAVING COUNT(*) >= 3
    ORDER BY COUNT(*) DESC
    LIMIT 1000;
...
BEGIN
  IF NOT EXISTS (
    ...
    PERFORM cron.schedule(
      ...
      '0 2 * * 0',
      'SET search_path = public; DELETE FROM refresh_metrics WHERE started_at < (now() - interval ''7 days'');'
    );
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    PERFORM cron.schedule(
      ...
      '*/10 * * * *',
      'SET search_path = public; UPDATE user_vibe_states SET active = FALSE WHERE active = TRUE AND started_at < (now() - interval ''90 minutes'');'
    );
  END IF;
END $$;