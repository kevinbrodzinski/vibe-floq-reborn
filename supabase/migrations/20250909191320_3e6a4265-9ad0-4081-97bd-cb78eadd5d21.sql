-- Migration 2: Geo helpers + RPC for precise convergence detection
BEGIN;

-- Optional but recommended for h3_idx support
CREATE EXTENSION IF NOT EXISTS h3;

-- If your venues table already has lng/lat columns, you can skip this view.
-- Otherwise, expose lng/lat from geometry for the search edge function.
-- Adjust table/column names as needed (geom SRID assumed 4326).
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM   information_schema.views
    WHERE  table_schema = 'public' AND table_name = 'v_venues_ll'
  ) THEN
    EXECUTE $v$
...
-- Recent convergence aggregator
-- Clusters by h3_idx when present; otherwise a coarse spatial grid and centroid of collected points.
-- Returns lng/lat + count for clusters intersecting the bbox since the provided timestamp.
...
agg AS (
  SELECT
    ...
    h3_idx,
    COUNT(*) AS n,
    /* If H3 available, convert cell to geocoord.
       Otherwise centroid of collected points in grid bucket. */
    CASE
      WHEN h3_idx IS NOT NULL AND EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'h3')
      THEN ST_X(ST_Centroid(ST_Collect(center)))::double precision
      ELSE ST_X(ST_Centroid(ST_Collect(center)))::double precision
    END AS lng,
    CASE
      WHEN h3_idx IS NOT NULL AND EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'h3')
      THEN ST_Y(ST_Centroid(ST_Collect(center)))::double precision
      ELSE ST_Y(ST_Centroid(ST_Collect(center)))::double precision
    END AS lat
  FROM bucketed
  GROUP BY 1,2
)
...
-- H3 trigger function for auto-populating h3_idx
CREATE OR REPLACE FUNCTION public.tg_flow_segments_set_h3()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
  r int := 9; -- Resolution 9 for city-scale clustering
  lng float8;
  lat float8;
BEGIN
  ...
  -- For now, create a simple grid-based key until H3 is fully available
  NEW.h3_idx := 'h3_' || round(lng::numeric, 4) || '_' || round(lat::numeric, 4);
