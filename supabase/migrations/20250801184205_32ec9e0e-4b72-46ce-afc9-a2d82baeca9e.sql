...
-- Add recommended indexes for better performance
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_friendships_pair_partial 
ON public.friendships (user_low, user_high) 
WHERE friend_state IN ('accepted', 'blocked');

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_friend_requests_out_partial
ON public.friend_requests (profile_id, other_profile_id) 
WHERE status = 'pending';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_friend_requests_in_partial
ON public.friend_requests (other_profile_id, profile_id) 
WHERE status = 'pending';

-- Fix the main function with proper aliases and precedence
