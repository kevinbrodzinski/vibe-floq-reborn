-- ============================================================================
-- Safe P2P Enhancement Migration - Step 1: Non-Conflicting Additions
...
-- Rate limiting index for friend requests (if not exists)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_indexes 
        WHERE indexname = 'idx_friend_requests_rate_limit'
    ) THEN
        CREATE INDEX idx_friend_requests_rate_limit
...
SELECT
    ...
    NOW() as completed_at,
    'Added non-conflicting views, indexes, and permissions' as description;