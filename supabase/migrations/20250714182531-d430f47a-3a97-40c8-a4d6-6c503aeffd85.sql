...
-- Phase 5: Create optimized indexes (without partial conditions)
CREATE INDEX idx_floq_messages_unread_tracking 
  ON public.floq_messages (floq_id, created_at DESC, sender_id);

CREATE INDEX idx_flock_history_unread_tracking 
  ON public.flock_history (floq_id, created_at DESC, user_id);

CREATE INDEX idx_floq_plans_unread_tracking 
  ON public.floq_plans (floq_id, created_at DESC, creator_id);
...
-- Phase 7: Bootstrap existing data (only valid users)
...
BEGIN
  -- Get total count for logging (only valid users)
  SELECT COUNT(*) INTO total_count
  FROM public.floq_participants fp
  INNER JOIN auth.users u ON u.id = fp.user_id
  WHERE NOT EXISTS (
  ...
  FROM public.floq_participants fp
  WHERE NOT EXISTS (SELECT 1 FROM auth.users u WHERE u.id = fp.user_id);
  
  RAISE NOTICE 'Initializing tracking for % valid participants (% orphaned records skipped)', total_count, orphaned_count;
  
  -- Process in batches to avoid long locks (only valid users)
  LOOP
    WITH batch AS (
