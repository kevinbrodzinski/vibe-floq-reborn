/* ---------------------------------------------------------------------
   🔒  Phase 4 – Critical Security Hardening  (PG 14 compatible)
   ------------------------------------------------------------------- */
BEGIN;
...
/* ------------------------------------------------------------------ */
/* 2. Pin search_path on SECURITY DEFINER functions (with error handling) */
/* ------------------------------------------------------------------ */
DO $$
DECLARE
  func_name text;
BEGIN
  ...
  LOOP
    BEGIN
      EXECUTE format('ALTER FUNCTION %s SET search_path = public, pg_catalog', func_name);
