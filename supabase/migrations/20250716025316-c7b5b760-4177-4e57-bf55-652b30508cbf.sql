...
CREATE INDEX IF NOT EXISTS idx_daily_afterglow_search_summary
    ON public.daily_afterglow
    USING gin (to_tsvector('english', coalesce(summary_text, '')));

CREATE INDEX IF NOT EXISTS idx_daily_afterglow_search_vibe
    ON public.daily_afterglow
    USING gin (to_tsvector('english', coalesce(dominant_vibe, '')));
