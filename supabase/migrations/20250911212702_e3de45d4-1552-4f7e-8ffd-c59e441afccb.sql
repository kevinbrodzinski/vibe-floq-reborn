...
-- Requires: profiles(id, display_name, avatar_url), flows(id, profile_id, started_at, ended_at),
--           flow_segments(flow_id, idx, arrived_at, center geometry(Point,4326))
...
AS $$
  ...
  tails AS (
    ...
    SELECT s.flow_id,
           array_agg(s.center ORDER BY s.t DESC)[:max_per_friend] AS pts_desc,
           max(s.t) AS t_head
    FROM seg s
    GROUP BY s.flow_id
  ),
    ...
    SELECT t.flow_id,
           ...
           ST_AsGeoJSON(
             ST_SimplifyPreserveTopology(
               ST_MakeLine(
                 CASE WHEN array_length(t.pts_desc, 1) > 1 
                   THEN (SELECT array_agg(p) FROM unnest(array_reverse(t.pts_desc)) AS p)
                   ELSE t.pts_desc
                 END
               ),
               0.0002
           ...
           ST_X(t.pts_desc[1]) AS head_lng,
           ST_Y(t.pts_desc[1]) AS head_lat
    FROM tails t
    WHERE array_length(t.pts_desc, 1) > 0
  )
