-- Phase 1: Database alignment for vibes_now table
-- Add computed valence/energy from vibe enum and optimize for field queries

-- Add function to compute valence/energy from vibe
CREATE OR REPLACE FUNCTION compute_vibe_vector(vibe_text text)
RETURNS jsonb
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
IMMUTABLE
AS $$
  -- This is a placeholder - in production you'd use actual H3 extension
  -- For now, create grid-based pseudo-H3 IDs
  SELECT CONCAT(
...
CREATE INDEX IF NOT EXISTS idx_vibes_now_expires_at ON vibes_now(expires_at) WHERE expires_at > now();
CREATE INDEX IF NOT EXISTS idx_vibes_now_visibility ON vibes_now(visibility) WHERE visibility = 'public';
CREATE INDEX IF NOT EXISTS idx_vibes_now_updated_at ON vibes_now(updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_vibes_now_location_gist ON vibes_now USING GIST(location) WHERE expires_at > now();

-- Composite index for field tile queries
CREATE INDEX IF NOT EXISTS idx_vibes_now_field_composite ON vibes_now(h3_idx_7, updated_at DESC, visibility) 
WHERE expires_at > now();

-- Function to get field tiles from vibes_now data
CREATE OR REPLACE FUNCTION get_field_tiles_from_vibes(
  h3_cells text[],
  time_window_minutes integer DEFAULT 10,
  min_crowd_count integer DEFAULT 1
)
RETURNS TABLE(
  tile_id text,
  crowd_count integer,
...
BEGIN
  ...
  SELECT 
    ...
    jsonb_build_object(
      'h', AVG((vn.vibe_vector->>'valence')::numeric * 180 + 180),
      's', AVG((vn.vibe_vector->>'energy')::numeric * 100),
      'l', GREATEST(20, LEAST(80, AVG((vn.vibe_vector->>'energy')::numeric * 60 + 40)))
    ) as avg_vibe,
    ARRAY_AGG(DISTINCT venue_id) FILTER (WHERE venue_id IS NOT NULL) as active_floq_ids,
