-- Drop the existing function first to allow type change
DROP FUNCTION IF EXISTS public.venues_near_me(NUMERIC, NUMERIC, NUMERIC);
...
-- Optimized venues_near_me function with proper SRID handling
...
AS $$
  ...
  nearby AS (
    SELECT v.id, v.name, v.vibe, v.source,
           v.lat, v.lng,
           v.geo AS geom
    FROM   public.venues v, user_point u
    WHERE  ST_DWithin(
             v.geo::geography,
             u.geom::geography,
             radius_km * 1000
  ...
  SELECT 
    ...
    n.vibe,
    n.source,
    ST_Distance(n.geom::geography, u.geom::geography)::integer AS distance_m,
