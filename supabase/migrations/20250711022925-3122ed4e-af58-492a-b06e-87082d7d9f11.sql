-- ===============================================
-- Achievement System Performance & Security Migration
...
CREATE OR REPLACE FUNCTION public.get_achievement_stats(
  IN  target_user_id uuid DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Enable row-level security on the function to force RLS inside SECURITY DEFINER
ALTER FUNCTION get_achievement_stats(uuid) SET ROW SECURITY ON;

-- 5️⃣ Optional Materialized View for Leaderboard (commented out - enable if needed)
-- Uncomment and setup pg_cron refresh for high-traffic leaderboards
/*
...
SELECT 
  ...
  ROUND(COUNT(*) FILTER (WHERE earned_at IS NOT NULL)::numeric / GREATEST(COUNT(*), 1) * 100, 1) AS completion_percentage,
...
-- Create index for rank-based queries  
CREATE INDEX IF NOT EXISTS idx_leaderboard_cache_rank 
  ON leaderboard_cache (user_rank);
*/

-- 6️⃣ Enhanced Achievement Progress Tracking Function
...
-- 7️⃣ Efficient Achievement Progress Query Function
...
-- 8️⃣ Performance monitoring view for achievement system
...
SELECT 
  ...
  ROUND(AVG(
    ...
    / (SELECT COUNT(*) FROM achievement_catalogue) * 100
  ), 2)::text as value
