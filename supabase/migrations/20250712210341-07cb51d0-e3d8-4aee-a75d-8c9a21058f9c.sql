...
-- Create find_or_create_dm with demo support
...
BEGIN
  ...
  EXECUTE format(
    'SELECT id FROM %I.direct_threads WHERE (member_a = $1 AND member_b = $2) OR (member_a = $2 AND member_b = $1)',
    schema_name
  ) INTO thread_id
  USING a, b;
  ...
  IF thread_id IS NULL THEN
    EXECUTE format(
      'INSERT INTO %I.direct_threads (member_a, member_b) VALUES (LEAST($1, $2), GREATEST($1, $2)) RETURNING id',
      schema_name
    ) INTO thread_id
    USING a, b;
  END IF;
...
-- Enable RLS on demo schema tables
...
CREATE POLICY "demo_direct_threads_public_read" ON demo.direct_threads FOR SELECT USING (true);
CREATE POLICY "demo_direct_messages_public_read" ON demo.direct_messages FOR SELECT USING (true);
...
GRANT SELECT, INSERT, UPDATE ON demo.direct_threads TO anon, authenticated;
GRANT SELECT, INSERT ON demo.direct_messages TO anon, authenticated;