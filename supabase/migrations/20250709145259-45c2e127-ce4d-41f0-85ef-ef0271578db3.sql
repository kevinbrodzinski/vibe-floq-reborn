...
-- 3. Drop existing function if it exists (parameter name change)
DROP FUNCTION IF EXISTS public.search_users(text);

-- 4. RPC: case-insensitive username search excluding current user and friends
...
-- 5. Set function ownership and grant permissions
ALTER FUNCTION public.search_users(text) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.search_users(text) TO authenticated;

-- 6. Simplified public read policy (relaxed for dev)
