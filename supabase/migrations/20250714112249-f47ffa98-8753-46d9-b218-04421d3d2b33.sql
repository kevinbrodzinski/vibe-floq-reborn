...
-- Add check constraint for pinned note length (drop first if exists)
DO $$
BEGIN
  -- Drop existing constraint if it exists
  IF EXISTS (SELECT 1 FROM information_schema.table_constraints 
             WHERE constraint_name = 'floqs_pinned_note_check' 
             AND table_name = 'floqs') THEN
    ALTER TABLE public.floqs DROP CONSTRAINT floqs_pinned_note_check;
  END IF;
  
  -- Add the constraint
  ALTER TABLE public.floqs ADD CONSTRAINT floqs_pinned_note_check 
...
-- RLS policy for users to read their own notifications (drop first if exists)
