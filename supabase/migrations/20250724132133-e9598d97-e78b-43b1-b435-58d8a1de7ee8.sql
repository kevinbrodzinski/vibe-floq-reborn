...
-- RLS is allowed on mat-views (they're tables underneath)
ALTER MATERIALIZED VIEW public.v_friend_sparkline ENABLE ROW LEVEL SECURITY;
CREATE POLICY own_rows
  ON public.v_friend_sparkline
  FOR SELECT
...
-- refresh every 10 min (idempotent re-scheduler)
DO $$
DECLARE jid int;
BEGIN
  SELECT jobid INTO jid FROM cron.job WHERE jobname = 'refresh_friend_sparkline';
  IF jid IS NOT NULL THEN           -- unschedule previous
  ...
  PERFORM cron.schedule(
    ...
    '*/10 * * * *',
    $$ REFRESH MATERIALIZED VIEW CONCURRENTLY public.v_friend_sparkline; $$
  );
END $$;