...
-- RLS Policies
DROP POLICY IF EXISTS "lp_select_owner" ON public.learned_preferences;
...
DROP POLICY IF EXISTS "lp_upsert_owner" ON public.learned_preferences;
...
DROP POLICY IF EXISTS "lp_update_owner" ON public.learned_preferences;
...
BEGIN
  v_profile := public.get_current_profile_id();
