...
CREATE POLICY friend_requests_participant_read
  ...
  FOR SELECT
  USING (profile_id = auth.uid() OR other_profile_id = auth.uid());

CREATE POLICY friend_requests_send_own
  ON public.friend_requests
  FOR INSERT
  WITH CHECK (profile_id = auth.uid() AND other_profile_id <> auth.uid());

CREATE POLICY friend_requests_respond
  ON public.friend_requests
  FOR UPDATE
  USING (profile_id = auth.uid() OR other_profile_id = auth.uid())
  WITH CHECK (profile_id = auth.uid() OR other_profile_id = auth.uid());
