-- Update refresh_field_tiles function to use user_vibe_states and include active floq IDs
...
BEGIN
  ...
  SELECT
    uvs.gh5,
    COUNT(*),
    -- Circular-mean hue + linear S/L (assuming vibe_h is 0-1)
    jsonb_build_object(
      'h', CASE 
        WHEN COUNT(*) > 0 THEN
...
-- Also update the test to check JSON hue value
-- Note: This replaces the existing test content
DO $$
BEGIN
  -- Update the pg-tap test to check the JSON hue value properly
  -- This would go in supabase/tests/field_tiles_refresh.sql
END$$;