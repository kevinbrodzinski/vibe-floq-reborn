...
active_now AS (
  ...
  WHERE vn.updated_at > now() - interval '30 minutes'  -- Use updated_at instead of created_at
  GROUP BY b.id
),
recent_msgs AS (
  SELECT
...
member_locs AS (
  ...
  LEFT JOIN vibes_now vn ON vn.profile_id = fp.profile_id
    AND vn.updated_at > now() - interval '30 minutes'  -- Use updated_at instead of created_at
  GROUP BY b.id
),
centroids AS (
  SELECT
