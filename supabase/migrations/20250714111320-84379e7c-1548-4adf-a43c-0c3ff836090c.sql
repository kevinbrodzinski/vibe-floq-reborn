...
-- 7. Update profiles RLS to protect push_token
-- (Existing profiles policies should already protect this, but let's be explicit)
CREATE POLICY IF NOT EXISTS "profiles_protect_push_token"
  ON public.profiles
  FOR SELECT
  USING ( 
    id = auth.uid() OR 
    (id != auth.uid() AND push_token IS NULL) -- Others can't see push_token
  );
