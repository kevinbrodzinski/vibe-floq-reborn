-- Improve rally mark read with server-side timestamps and deduplication
...
-- Add unique index for story message deduplication (1/min per author/kind)
create unique index if not exists uq_rally_story_minute
  on rally_messages(thread_id, kind, profile_id, date_trunc('minute', created_at))
  where kind != 'user';