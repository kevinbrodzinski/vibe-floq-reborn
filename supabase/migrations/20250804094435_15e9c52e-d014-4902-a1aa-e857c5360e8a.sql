...
--   b) participants may update/delete
CREATE POLICY friendships_participant_modify
  ON public.friendships
  FOR UPDATE, DELETE
  USING (user_id = auth.uid() OR friend_id = auth.uid());

--   c) either participant may create the row
...
BEGIN
  ...
  FROM   (VALUES
          ...
          ('friendships','friendships_participant_modify'),
