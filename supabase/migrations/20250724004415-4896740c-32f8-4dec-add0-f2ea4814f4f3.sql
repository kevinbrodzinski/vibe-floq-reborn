...
BEGIN
  ...
  LOOP
    ...
    IF has_uid THEN
      EXECUTE format(
        'CREATE POLICY "own_records_user_id" ON public.%I FOR ALL USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid())',
        t);
    ELSIF has_creator THEN
      EXECUTE format(
        'CREATE POLICY "own_records_creator_id" ON public.%I FOR ALL USING (creator_id = auth.uid()) WITH CHECK (creator_id = auth.uid())',
        t);
    ELSE
      EXECUTE format(
        'CREATE POLICY "authenticated_readonly" ON public.%I FOR SELECT USING (auth.uid() IS NOT NULL)',
        t);
    END IF;
  END LOOP;
END $$;
