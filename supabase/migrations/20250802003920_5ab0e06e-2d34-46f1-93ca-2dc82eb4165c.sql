-- Step 1A: Updated clustering database layer aligned with actual table structure

/* ------------------------------------------------------------
   1.  HELPER: clamp precision into 4-9 range
------------------------------------------------------------ */
...
SELECT
  ...
  ST_SetSRID(
    ST_MakePoint(
      -- Extract coordinates from H3 tile_id if available, otherwise use simple grid
      CASE 
        WHEN length(ft.tile_id) >= 10 THEN 
          COALESCE(ST_X(ST_Centroid(h3_cell_to_boundary_geometry(ft.tile_id))), 0.0)
        ELSE 
          -- Fallback: parse as simple grid coordinate
          (substring(ft.tile_id from 1 for 3)::numeric - 500) * 0.001
      END,
      CASE 
        WHEN length(ft.tile_id) >= 10 THEN 
          COALESCE(ST_Y(ST_Centroid(h3_cell_to_boundary_geometry(ft.tile_id))), 0.0)
        ELSE 
          (substring(ft.tile_id from 4 for 3)::numeric - 500) * 0.001
      END
    ),
    4326
  ...
  'chill'                                AS vibe_mode  -- Default since avg_vibe is HSL format
...
/* ------------------------------------------------------------
   4.  FIXED API FUNCTION: get_vibe_clusters
------------------------------------------------------------ */
    ...
    WHERE  crowd_count > 0
      AND  ST_Intersects(
             ST_SetSRID(ST_MakePoint(
               -- Same coordinate extraction logic as view
               CASE 
                 WHEN length(tile_id) >= 10 THEN 
                   COALESCE(ST_X(ST_Centroid(h3_cell_to_boundary_geometry(tile_id))), 0.0)
                 ELSE 
                   (substring(tile_id from 1 for 3)::numeric - 500) * 0.001
               END,
               CASE 
                 WHEN length(tile_id) >= 10 THEN 
                   COALESCE(ST_Y(ST_Centroid(h3_cell_to_boundary_geometry(tile_id))), 0.0)
                 ELSE 
                   (substring(tile_id from 4 for 3)::numeric - 500) * 0.001
               END
             ), 4326),
             bbox)
  ),
