-- Rich mentions system for floq chat
-- Phase 0: Core schema + automatic parsing

-- 1. Create mention target enum
CREATE TYPE public.mention_target AS ENUM ('user', 'venue', 'plan');

-- 2. Create floq_message_mentions table
...
-- 3. Enable RLS and inherit from floq_messages permissions
...
-- 4. Helper function for slug-to-id lookup
...
-- 5. Mention parsing trigger function
...
-- 8. Add slug column to venues if it doesn't exist (for mention lookup)
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    UPDATE public.venues SET slug = LOWER(REGEXP_REPLACE(name, '[^a-zA-Z0-9]+', '-', 'g'));
  END IF;
END
$$;