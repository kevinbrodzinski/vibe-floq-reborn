-- Rally inbox system with unread counts and message threading
...
-- Helper function to check rally membership
CREATE OR REPLACE FUNCTION public.is_rally_member(_rally UUID, _uid UUID DEFAULT auth.uid())
RETURNS BOOLEAN
LANGUAGE sql STABLE
AS $$
  SELECT EXISTS (
...
LANGUAGE sql STABLE AS $$
  SELECT r.id AS rally_id, r.creator_id AS profile_id, 'creator'::TEXT AS role
  FROM public.rallies r
  WHERE r.status = 'active'
  UNION ALL
  SELECT i.rally_id, i.to_profile, 'invitee'::TEXT
...
stats AS (
  SELECT
    ...
    (SELECT COUNT(*) FROM public.rally_invites ri
      WHERE ri.rally_id = b.rally_id AND ri.status = 'joined') AS joined_count,
    (SELECT COUNT(*) FROM public.rally_invites ri
      WHERE ri.rally_id = b.rally_id AND ri.status = 'pending') AS pending_count
  FROM _base b
)
SELECT
  b.profile_id,
...
REVOKE ALL ON FUNCTION public.is_rally_member(UUID, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.is_rally_member(UUID, UUID) TO authenticated, anon;
