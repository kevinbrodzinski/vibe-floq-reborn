-- Pure-SQL Haversine friction calculator (PostGIS-free)
-- Matches edge function constants exactly for consistent scoring
...
AS $fn$
  ...
  scored AS (
    SELECT
      ...
      f.social,
      f.financial,
      -- friction = 0.30*coord + 0.40*logistics + 0.15*social + 0.15*financial
      (0.30*f.coordination + 0.40*f.logistics + 0.15*f.social + 0.15*f.financial) AS friction
    FROM factors f
  )
  SELECT
    path_id,
  ...
  FROM scored
  ORDER BY friction ASC, meters ASC;
