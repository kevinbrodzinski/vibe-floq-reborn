...
BEGIN
  IF (TG_OP = 'INSERT'
    ...
    ON CONFLICT (user_id, started_at::date)
      DO UPDATE SET
...
CREATE TABLE IF NOT EXISTS public.user_proximity_events (
  ...
  CONSTRAINT uniq_pair_day UNIQUE (
...
CREATE INDEX IF NOT EXISTS idx_vlp_user_day
  ON public.venue_live_presence (user_id, date_trunc('day',last_heartbeat));
CREATE INDEX IF NOT EXISTS idx_fp_user_day
  ON public.floq_participants (user_id, date_trunc('day',joined_at));
