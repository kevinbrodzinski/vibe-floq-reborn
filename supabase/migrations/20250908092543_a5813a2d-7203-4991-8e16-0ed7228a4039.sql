...
AS $$
  ...
  -- RSVP weights (using current text values: 'pending', 'confirmed', 'declined')
  pair_rsvp AS (
    SELECT
      p.a, p.b,
      CASE WHEN ra.rsvp_status='confirmed' AND rb.rsvp_status='confirmed' THEN 0.40
           WHEN ra.rsvp_status='pending'    AND rb.rsvp_status='pending'    THEN 0.20
