...
-- safely remove existing cleanup job if it exists
do $$
begin
  if exists (select 1 from cron.job where jobname = 'cleanup_field_tiles_60s') then
    perform cron.unschedule('cleanup_field_tiles_60s');
  end if;
end;
$$;
