...
/*───────────────────────────────────────────────────────────────────────────
  2.  DROP objects that depend on profiles.username
      – profiles_username_lowercase_trigger
      – profiles_prevent_reserved_usernames
      – profiles_username_conflict_monitor      ← **new**
      – view  v_friends_with_profile   (cached)
───────────────────────────────────────────────────────────────────────────*/
do $$
declare
    vdef text;
begin
    -- Helper to drop a trigger if it exists
    perform
        case
            when exists (
                select 1
    ...
    perform
        case
            when exists (
                select 1
                from pg_trigger
                where tgname = 'profiles_prevent_reserved_usernames'
                  and tgrelid = 'public.profiles'::regclass
            ) then
                execute 'drop trigger profiles_prevent_reserved_usernames on public.profiles';
        end;
    ...
    perform
        case
            when exists (
                select 1
                from pg_trigger
                where tgname = 'profiles_username_conflict_monitor'
                  and tgrelid = 'public.profiles'::regclass
            ) then
                execute 'drop trigger profiles_username_conflict_monitor on public.profiles';
        end;

    -- Stash + drop the view if present
    if to_regclass('public.v_friends_with_profile') is not null then
        select pg_get_viewdef('public.v_friends_with_profile'::regclass, true)
          into vdef;
        execute 'drop view public.v_friends_with_profile';
        create temporary table if not exists _tmp_viewdef (v text) on commit drop;
...
/*───────────────────────────────────────────────────────────────────────────
  3.  PROFILES  (ensure table, force username → CITEXT)
───────────────────────────────────────────────────────────────────────────*/
...
/*───────────────────────────────────────────────────────────────────────────
  5.  SUPPORT TABLE  (reserved handles, if you use it)
───────────────────────────────────────────────────────────────────────────*/
...
for each row execute function public.ensure_username_lowercase();
...
begin
  if exists (
      select 1 from public.reserved_usernames where name = new.username
  ) then
...
create trigger profiles_prevent_reserved_usernames
before insert or update on public.profiles
for each row execute function public.prevent_reserved_usernames();

/*───────────────────────────────────────────────────────────────────────────
  7.  AUTO-PROFILE  (new user hook)
───────────────────────────────────────────────────────────────────────────*/
...
/*───────────────────────────────────────────────────────────────────────────
  9.  RE-CREATE view (if we cached it)
───────────────────────────────────────────────────────────────────────────*/
do $$
declare vdef text;
begin
