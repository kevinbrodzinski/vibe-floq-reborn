...
-- 1. Add foreign key constraint for plan_participants.user_id (with proper syntax)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'fk_plan_participants_user_id' 
    AND table_name = 'plan_participants'
  ) THEN
    ALTER TABLE public.plan_participants 
    ADD CONSTRAINT fk_plan_participants_user_id 
