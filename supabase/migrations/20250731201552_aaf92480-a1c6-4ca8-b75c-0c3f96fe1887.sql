-- 2025-08-01 · Messaging patch 1 (fix FK + enable RLS) - CORRECTED
...
------------------------------------------------------------
-- 2 ▸ Required helper: user_in_floq_or_creator (fixed column name)
------------------------------------------------------------
...
AS $$
  SELECT EXISTS (
    ...
    WHERE fp.id = p_plan_id
      AND (
        fp.creator_id = p_user_id OR      -- plan creator (corrected column name)
