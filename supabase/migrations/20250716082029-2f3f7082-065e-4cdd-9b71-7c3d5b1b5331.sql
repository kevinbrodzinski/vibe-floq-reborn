...
-- owner full access (already likely exists; ensure it's there)
create policy if not exists p_afterglow_owner_full
...
create policy if not exists p_afterglow_public_read
  on public.daily_afterglow
  for select
