...
-- Add some recent venue stays for historical activity
INSERT INTO public.venue_stays (profile_id, venue_id, arrived_at, departed_at, vibe)
SELECT 
  (SELECT id FROM public.profiles ORDER BY random() LIMIT 1) as profile_id,
  v.id as venue_id,
  now() - (random() * interval '3 days') as arrived_at,
  now() - (random() * interval '3 days') + (random() * interval '2 hours') as departed_at,
  (ARRAY['chill', 'social', 'energetic', 'focused', 'excited'])[floor(random() * 5 + 1)]::public.vibe_enum as vibe
FROM public.venues v
ORDER BY random()
LIMIT 25  -- Add historical stays for 25 venues
