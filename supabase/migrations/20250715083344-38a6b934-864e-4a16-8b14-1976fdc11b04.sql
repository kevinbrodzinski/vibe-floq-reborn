...
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS uniq_user_vibe_active 
ON public.user_vibe_states(user_id) WHERE active;

-- Create composite index for MV refresh (active rows with started_at)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_vibe_states_active_started 
ON public.user_vibe_states(started_at) WHERE active;

-- Optional vibe-based index for analytics
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_vibe_states_vibe 
...
BEGIN
  ...
  ON CONFLICT (user_id) WHERE active DO UPDATE
    SET vibe_tag = EXCLUDED.vibe_tag,
        location = COALESCE(EXCLUDED.location, public.user_vibe_states.location),
        started_at = EXCLUDED.started_at,
        active = TRUE,
        visible_to = COALESCE(public.user_vibe_states.visible_to, 'public');
...
ANALYZE public.user_vibe_states;