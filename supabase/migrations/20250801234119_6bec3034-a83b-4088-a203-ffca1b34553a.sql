-- Phase 2: Generate Test Data for Afterglow System (Simplified)
...
BEGIN
  ...
  IF test_user_id IS NOT NULL THEN
    ...
    -- Create some raw location data (only required columns)
    FOR i IN 0..20 LOOP
...
-- Verify we have data by checking the counts
DO $$
DECLARE
  venue_count int;
  visit_count int;
  ...
  location_count int;
BEGIN
  SELECT COUNT(*) INTO venue_count FROM public.venues WHERE provider = 'manual';
  SELECT COUNT(*) INTO visit_count FROM public.venue_visits;
  SELECT COUNT(*) INTO stay_count FROM public.venue_stays;
  SELECT COUNT(*) INTO location_count FROM public.raw_locations;
  
  RAISE NOTICE 'Data created - Venues: %, Visits: %, Stays: %, Locations: %', 
    venue_count, visit_count, stay_count, location_count;
END $$;