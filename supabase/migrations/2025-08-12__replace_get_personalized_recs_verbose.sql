-- 2025-08-12__replace_get_personalized_recs_verbose.sql
begin;

-- Drop the old signature exactly (no badges in return type)
drop function if exists public.get_personalized_recs_verbose(
  uuid,        -- p_profile_id
  double precision, -- p_lat
  double precision, -- p_lng
  integer,     -- p_radius_m
  timestamp with time zone, -- p_now
  text,        -- p_vibe
  text[],      -- p_tags
  text,        -- p_tz
  integer,     -- p_limit
...
-- Recreate with badges[] and fixed logging scope
create function public.get_personalized_recs_verbose(
  p_profile_id uuid,
...
begin
  ...
  w as (
    select
      coalesce( (ut.model_weights->>'w_distance')::numeric,
                (uw.weights->>'w_distance')::numeric,
                vw.w_distance, dv.w_distance) as w_distance,
      coalesce( (ut.model_weights->>'w_rating')::numeric,
                (uw.weights->>'w_rating')::numeric,
                vw.w_rating, dv.w_rating) as w_rating,
      coalesce( (ut.model_weights->>'w_popularity')::numeric,
                (uw.weights->>'w_popularity')::numeric,
                vw.w_popularity, dv.w_popularity) as w_popularity,
      coalesce( (ut.model_weights->>'w_tag_match')::numeric,
                (uw.weights->>'w_tag_match')::numeric,
                vw.w_tag_match, dv.w_tag_match) as w_tag_match,
      coalesce( (ut.model_weights->>'w_cuisine_match')::numeric,
                (uw.weights->>'w_cuisine_match')::numeric,
                vw.w_cuisine_match, dv.w_cuisine_match) as w_cuisine_match,
      coalesce( (ut.model_weights->>'w_price_fit')::numeric,
                (uw.weights->>'w_price_fit')::numeric,
                vw.w_price_fit, dv.w_price_fit) as w_price_fit,
  ...
  -- ===== logging (recompute local context; no out-of-scope refs) =====
  if p_log and p_profile_id is not null then
    ...
    select
      ...
      (select array_agg(venue_id order by score desc) from ranked),
      (select array_agg(score     order by score desc) from ranked),
      (select array_agg(venue_id  order by score desc) from ranked),
      p_ab;
  end if;
end;
