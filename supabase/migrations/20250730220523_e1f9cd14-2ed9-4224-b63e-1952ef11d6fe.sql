/* ════════════════════════════════════════════════════════════════════════
   Phase-2  Security Hardening  ―  Idempotent Migration (CORRECTED)
...
------------------------------------------------------------------------
-- 2c  presence   (owner-only access, no visibility column exists)
------------------------------------------------------------------------
DO $$
DECLARE
  pol TEXT;
BEGIN
  ...
  /* owner full control only - presence table doesn't have visibility */
...
-- 4a upsert_presence (updated to match actual presence table structure)
...
AS $$
DECLARE
  v_row public.presence%rowtype;
BEGIN
  IF p_lat IS NULL OR p_lng IS NULL THEN
     RAISE EXCEPTION 'Latitude and longitude required';
  END IF;

  INSERT INTO public.presence AS p
      (profile_id, venue_id, vibe, location, lat, lng, updated_at, started_at)
  VALUES (p_profile_id,
          p_venue_id,
          p_vibe,
...
-- 4b award_achievement_optimized  (full finished body)
