...
energy AS (
  SELECT b.id AS floq_id,
         LEAST(1.0, GREATEST(0.0, COALESCE(b.activity_score,0)/10.0 + COALESCE(rm.activity_score,0)/12.0)) AS energy
  FROM base b
...
SELECT
  ...
  np.next_plan_id,                         -- ◀ expose plan id
  np.next_label,
  nw.next_when,
...
LEFT JOIN following      f   ON f.floq_id    = b.id
LEFT JOIN streak         s   ON s.floq_id    = b.id;