...
CREATE TABLE IF NOT EXISTS public.place_banners (
  ...
  -- Pre-computed geohash channel (populated by trigger)
  channel text
);
...
-- Function to update channel based on venue location
CREATE OR REPLACE FUNCTION update_banner_channel()
RETURNS TRIGGER AS $$
BEGIN
  -- Set geohash channel from venue location
  SELECT substring(st_geohash(v.location, 4)::text, 1, 4)
  INTO NEW.channel
  FROM venues v 
...
-- Trigger to auto-populate channel
CREATE TRIGGER set_banner_channel
  BEFORE INSERT OR UPDATE ON public.place_banners
  FOR EACH ROW
  EXECUTE FUNCTION update_banner_channel();
...
BEGIN
  ...
  -- Insert banner (trigger will set channel)
