-- Update floqs_card_view with energy clamping and improved logic
...
-- RALLY: support both cases - direct floq_id or via invites
rally_direct AS (
  SELECT r.floq_id,
  ...
  WHERE r.created_at > now() - interval '24 hours'
    AND r.floq_id IS NOT NULL
  GROUP BY r.floq_id
),
rally_by_invites AS (
  -- Map rallies to floqs via participants who were invited
  SELECT fp.floq_id,
         BOOL_OR(r.status = 'active') AS rally_now,
         BOOL_OR(r.status = 'forming') AS forming
  FROM rallies r
  ...
  WHERE r.created_at > now() - interval '24 hours'
    AND r.floq_id IS NULL
  GROUP BY fp.floq_id
),
rally AS (
  SELECT b.id AS floq_id,
         COALESCE(rd.rally_now, ri.rally_now, FALSE) AS rally_now,
         COALESCE(rd.forming, ri.forming, FALSE) AS forming
  FROM base b
  LEFT JOIN rally_direct rd ON rd.floq_id = b.id
...
CREATE INDEX IF NOT EXISTS idx_uos_last_seen ON user_online_status(last_seen DESC);
CREATE INDEX IF NOT EXISTS idx_vibes_now_loc ON vibes_now USING GIST (location);
CREATE INDEX IF NOT EXISTS idx_vibes_now_updated ON vibes_now(updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_rallies_time ON rallies(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_rally_invites ON rally_invites(rally_id, to_profile);
CREATE INDEX IF NOT EXISTS idx_plans_floq_start ON floq_plans(floq_id, planned_at);
CREATE INDEX IF NOT EXISTS idx_msgs_floq_time ON floq_messages(floq_id, created_at DESC);