-- Drop and recreate get_profile_stats function to ensure latest version
...
BEGIN
  ...
  -- Most active vibe (fallback to 'social') - use vibe_tag column
  SELECT   COALESCE(vibe_tag::text, 'social')
  ...
  -- Vibe distribution (last 30 days) - use vibe_tag column
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'vibe', vibe_tag,
  ...
  -- Recent vibes (last 10) - use vibe_tag column
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'vibe', vibe_tag::text,
