...
ALTER TABLE public.venues
  ADD COLUMN IF NOT EXISTS price_tier price_enum DEFAULT '$';
...
BEGIN
  ...
  SELECT
    ...
    v.address,
    v.photo_url,
    COALESCE(vps.people_now, 0)             AS live_count,
    COALESCE(v.price_tier,'$')              AS price_tier,
    CASE
      WHEN p_profile_id IS NOT NULL THEN
        get_personalized_venue_score(p_profile_id, v.venue_id,
                                     COALESCE(v.rating/5.0,0.5))
      ELSE
        COALESCE(v.rating/5.0,0.5)
    END                                     AS personalized_score
  FROM   public.venues v
  LEFT   JOIN public.venue_presence_snapshot vps
         ON v.venue_id = vps.venue_id
  WHERE  ST_DWithin(
           v.geom::geography,
           ST_SetSRID(ST_MakePoint(p_lng, p_lat), 4326)::geography,
           p_radius_m
    ...
    AND (p_vibe IS NULL
         OR vps.dominant_vibe = p_vibe
         OR p_vibe = ANY(v.categories))
