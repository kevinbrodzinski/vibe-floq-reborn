-- Phase 2: People Discovery Stack Data Infrastructure (Improved Version)
-- Based on code review feedback with security and performance improvements

BEGIN;

-- 1. Performance index for venue visits
CREATE INDEX IF NOT EXISTS idx_venue_visits_profile_recent
  ON public.venue_visits (profile_id, visited_at DESC);
...
BEGIN
  -- Overall score: use existing vibe_similarity if available, fallback to venue overlap
  BEGIN
    SELECT vibe_similarity(me_id, friend_id) * 100 INTO overall_score;
  EXCEPTION WHEN OTHERS THEN
    -- Fallback: venue-based similarity
    SELECT 
      COUNT(DISTINCT CASE WHEN v1.venue_id IS NOT NULL THEN v1.venue_id END),
    ...
    FULL OUTER JOIN venue_visits v2 
     ...
     AND v1.profile_id = me_id  
     AND v2.profile_id = friend_id
    WHERE COALESCE(v1.visited_at, v2.visited_at) > now() - interval '30 days';
  ...
  WHERE v1.profile_id = me_id
    AND v2.profile_id = friend_id
    AND v1.visited_at > now() - interval '30 days'
  ...
  SELECT COALESCE(AVG(
    CASE WHEN ((EXTRACT(hour FROM v1.visited_at)::int -
                EXTRACT(hour FROM v2.visited_at)::int + 24) % 24) <= 3
         THEN 80 ELSE 20 END), 50.0)
  INTO time_rhythm_score
  FROM venue_visits v1
  JOIN venue_visits v2 
    ON v1.profile_id = me_id
   AND v2.profile_id = friend_id
   AND v1.visited_at > now() - interval '14 days'
   AND v2.visited_at > now() - interval '14 days'
...
BEGIN
  ...
  WHERE v1.profile_id = me_id
    AND v2.profile_id = friend_id
    AND v1.visited_at > now() - interval '60 days'
    AND v2.visited_at > now() - interval '60 days'
...
AS $$
DECLARE
  shared_vibe text := 'social';
BEGIN
  -- Get most common shared vibe from recent activity
  SELECT COALESCE(
    (SELECT mode() WITHIN GROUP (ORDER BY COALESCE(uvs1.current_vibe, uvs2.current_vibe))
     FROM user_vibe_states uvs1, user_vibe_states uvs2
     WHERE uvs1.profile_id = me_id AND uvs2.profile_id = friend_id
     AND uvs1.updated_at > now() - interval '7 days'
     AND uvs2.updated_at > now() - interval '7 days'),
    'social'
  ) INTO shared_vibe;

  RETURN QUERY
  SELECT 
    ('suggestion-' || generate_random_uuid()::text) as id,
    suggestion.title,
    suggestion.vibe,
  ...
  FROM (
    VALUES 
      ('Grab coffee and catch up', shared_vibe, 'cafe', '45-60 minutes'),
