-- Phase 3.1: Production-Safe Username Schema Migration (Final Fix)
-- Handle view dependencies before altering column types
...
-- 3. Store view definition and drop dependent view
CREATE OR REPLACE VIEW v_friends_with_profile_backup AS 
SELECT * FROM v_friends_with_profile;

DROP VIEW IF EXISTS v_friends_with_profile;
...
CREATE OR REPLACE VIEW v_friends_with_profile AS
SELECT  
...
-- 10. Mirror all changes to demo schema
CREATE SCHEMA IF NOT EXISTS demo;

-- Create demo.profiles table with citext username
...
-- Create demo.reserved_usernames table with citext
CREATE TABLE IF NOT EXISTS demo.reserved_usernames (
  name citext NOT NULL,
  PRIMARY KEY (name)
);

-- Add constraints to demo schema
...
-- 11. Add safety trigger to ensure data consistency
...
-- 12. Add performance indexes for search operations
...
-- 13. Add monitoring function for username conflicts
...
-- 14. Populate demo.reserved_usernames with test data
...
-- Add RLS policies for demo schema
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 16. Clean up backup view
DROP VIEW IF EXISTS v_friends_with_profile_backup;