-- Drop the friends table and fix search_floqs function (with proper function drop)
BEGIN;

-- 1. Drop the friends table (no longer used, replaced by friendships)
DROP TABLE IF EXISTS public.friends CASCADE;

-- 2. Drop the existing search_floqs function first
DROP FUNCTION IF EXISTS public.search_floqs(double precision, double precision, double precision, text, text[], timestamp with time zone, timestamp with time zone, integer);

-- 3. Recreate the search_floqs function to use friendships table correctly
