...
-- Migration: 20250802_fix_venue_schema_v2
...
BEGIN
  ...
  -- Check if geom exists and is generated - if not, add it as regular geometry column
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- Step 4: Only populate geom if it's not a generated column and is null
UPDATE public.venues
...
-- Step 8: Create composite index for vibe + location queries (only if geom exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    ...
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_venues_vibe_geom_gist ON public.venues USING GIST(geom, vibe)';
  END IF;
END $$;
