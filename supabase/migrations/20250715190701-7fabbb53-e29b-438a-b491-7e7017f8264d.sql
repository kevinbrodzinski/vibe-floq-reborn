...
-- ping policies
create policy if not exists ping_read_own_or_target
  on public.ping_requests
  for select
  using (auth.uid() in (requester_id, target_id));

create policy if not exists ping_insert_self_requester
  on public.ping_requests
  for insert
  with check (requester_id = auth.uid());

create policy if not exists ping_update_target_only
  on public.ping_requests
  for update
  using (target_id = auth.uid());

create policy if not exists ping_update_requester_cancel
  on public.ping_requests
  for update
  using (requester_id = auth.uid() and status = 'pending');

-- pin policies
create policy if not exists pin_viewer_can_see
  on public.shared_location_pins
  for select
  using (viewer_id = auth.uid());

create policy if not exists pin_owner_insert
  on public.shared_location_pins
  for insert
  with check (owner_id = auth.uid());

create policy if not exists pin_owner_delete
  on public.shared_location_pins
  for delete
