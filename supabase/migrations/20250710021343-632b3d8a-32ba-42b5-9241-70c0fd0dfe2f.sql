...
-- Fixed: Use ST_Buffer instead of ST_Expand for geography types
...
-- Create the refined function with corrected geography operations
  ...
  bounding_box_filter AS (
    ...
    WHERE vl.ts >= CURRENT_DATE
      ...
      AND ST_DWithin(
        ...
        mll.geog,
        proximity_meters * 1.5  -- Slightly larger bounding box for efficiency
      )
  ),
