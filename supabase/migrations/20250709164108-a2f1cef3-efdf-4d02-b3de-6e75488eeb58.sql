...
AS $$
BEGIN
  -- Only check if venue_id is set and location actually changed
  IF NEW.venue_id IS NOT NULL AND (OLD.location IS DISTINCT FROM NEW.location) THEN
    -- Check if user is still within venue radius
    IF NOT EXISTS (
      SELECT 1 FROM public.venues v
      WHERE v.id = NEW.venue_id
      AND ST_DWithin(v.geo::geography, NEW.location::geography, COALESCE(v.radius_m, 100))
    ) THEN
      NEW.venue_id := NULL;
    END IF;
  END IF;
...
-- Create trigger with WHEN clause for performance - fires on location and venue_id changes
...
CREATE TRIGGER trigger_clear_venue_id_on_distance
  BEFORE UPDATE OF location, venue_id ON public.vibes_now
  FOR EACH ROW
  WHEN (OLD.location IS DISTINCT FROM NEW.location OR OLD.venue_id IS DISTINCT FROM NEW.venue_id)
