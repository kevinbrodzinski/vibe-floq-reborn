-- Update floqs_card_view with correct column names and rally detection
...
active_now AS (
  ...
  FROM public.floq_participants fp
  JOIN user_online_status uos ON uos.profile_id = fp.profile_id
  WHERE uos.is_online = TRUE OR uos.last_seen > (now() - interval '5 minutes')
  GROUP BY fp.floq_id
),
-- Live locations in last 20 minutes for convergence
member_locs AS (
...
-- Energy calculation from activity
energy AS (
  SELECT b.id AS floq_id,
         LEAST(1.0, GREATEST(0.0,
           COALESCE(b.activity_score,0)/10.0
         )) AS energy
  FROM base b
),
-- Next upcoming plan (for RSVP)
next_plan AS (
...
next_when AS (
  SELECT floq_id,
         next_plan_id,
         CASE
           WHEN date_trunc('day', starts_at) = date_trunc('day', now()) THEN 'Today ' || to_char(starts_at, 'HH24:MI')
           WHEN date_trunc('day', starts_at) = date_trunc('day', now() + interval '1 day') THEN 'Tomorrow ' || to_char(starts_at, 'HH24:MI')
           WHEN starts_at < now() + interval '7 days' THEN to_char(starts_at, 'Dy HH24:MI')
           ELSE to_char(starts_at, 'Mon DD')
         END AS next_when
  FROM next_plan
),
-- Rally detection by invites to floq participants
rally_by_invites AS (
  SELECT fp.floq_id,
...
status_bucket AS (
  ...
  LEFT JOIN rally_by_invites r ON r.floq_id = b.id
)
SELECT
  b.id,
  b.name,
