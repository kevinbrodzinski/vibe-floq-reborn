...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: sl, error } = await supa.from('venue_shortlists')
      .select('id,name,created_at,venue_shortlist_items(venue_id)').order('created_at',{ascending:false}).limit(20)
    if (error) throw error
    return ok({ shortlists: sl ?? [] })
  } catch (e) { return ok({ shortlists: [] }, 10) }
})