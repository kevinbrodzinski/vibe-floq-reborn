...
const getMapboxToken = async (): Promise<{ token: string; source: string }> => {
  // First try: Edge function with FLOQ_PROD_2025 token
  try {
    const { data, error } = await supabase.functions.invoke('mapbox-token');
    if (data?.token && !error) {
      console.log('[WebMap] Using FLOQ_PROD_2025 token from edge function');
      return { token: data.token, source: 'edge-function' };
    }
    console.warn('[WebMap] Edge function failed:', error);
  } catch (e) {
    console.warn('[WebMap] Edge function unavailable:', e);
  }
  ...
  if (envToken) {
    console.log('[WebMap] Using MAPBOX_ACCESS_TOKEN from environment');
    return { token: envToken, source: 'environment' };
  }

  // Final fallback: Public token
  console.warn('[WebMap] Using fallback public token');
  return { 
