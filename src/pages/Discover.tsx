...
import WaveBottomSheet from '@/components/discovery/WaveBottomSheet';
import { createMomentaryFromWave } from '@/lib/createFloqFromWave';

function PlatformMap({ lat, lng, markers, onSelect }: { 
  lat: number; 
  lng: number; 
  markers: any[]; 
  onSelect: (m: any) => void;
  ...
  return isWeb ? (
    ...
    <WaveMapNative lat={lat} lng={lng} markers={markers} onSelect={onSelect} />
  );
}
...
export default function Discover() {
  ...
  const [selectedWave, setSelectedWave] = useState<{ id: string; size: number; friends: number; lat: number; lng: number } | null>(null);
  const [isCreating, setIsCreating] = useState(false);

  const openSheetFor = useCallback((marker: any) => {
    setSelectedWave(marker);
    setSheetOpen(true);
  }, []);
  ...
  const onLetsFloq = useCallback(async (payload: { venueId?: string; name?: string; lat: number; lng: number }) => {
    ...
    try {
      ...
      if ('floqId' in res && res.floqId) {
        // navigate to floq route
        window.location.href = `/floqs/${res.floqId}`;
      } else {
        console.error('Failed to create floq:', res);
      }
    } catch (error) {
      console.error('Exception creating floq:', error);
    } finally {
      setIsCreating(false);
      setSheetOpen(false);
    }
  ...
  return (
    <div className="flex flex-col h-full p-3 space-y-3">
      ...
      <div className="h-[300px]">
        <PlatformMap lat={lat} lng={lng} markers={markers} onSelect={openSheetFor} />
      </div>
