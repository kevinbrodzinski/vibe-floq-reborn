...
type Node = { id: string; pos: [number, number]; mass?: number; vibe?: string } // 0..1 screen-space
...
type LinkRec = {
  ...
  width: number
  steps: number
  colors: number[]   // precomputed
  alphas: number[]   // precomputed
  widths: number[]   // precomputed
}
...
export class ConstellationLinksPixi {
  ...
  private rebuild(nodes: Node[], edges: Edge[]) {
    ...
    for (const n of nodes) {
      ...
      s.anchor.set(0.5); s.tint = 0xffffff; s.alpha = 0.92
      const sz = 3.5 + ((n.mass ?? 1) * 1.5)
      s.width = sz; s.height = sz
      s.blendMode = 'add'
      this.stage.addChild(s)
      this.stars.push({ spr: s, id: n.id })
    }
    ...
    for (const e of this.edges) {
      ...
      this.links.push({ g, aId: a.id, bId: b.id, width: 1 + 2*s, steps: STEPS, colors: cols, alphas, widths })
    }
  }
...
// one-time hex interpolation (not per-frame)
function interpHex(a: string, b: string, t: number) {
  const [r1,g1,bl1] = hexToRgb(a); const [r2,g2,bl2] = hexToRgb(b)
