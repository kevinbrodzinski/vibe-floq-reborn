...
type Props = {
  ...
  onRallyHere?: (name: string) => void;
  onHistory?: (name: string) => void;
};

export default function VenuesTab({ reduce, panelAnim, onNavigate, onRallyHere, onHistory }: Props) {
  return (
    <motion.div {...panelAnim(reduce)} className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex gap-2 flex-wrap">
          <Btn active glow>All</Btn>
          <Btn>Day</Btn>
          <Btn>Night</Btn>
          <Btn>Food</Btn>
          <Btn>Bars</Btn>
        ...
        {VENUES.map((v) => (
          <div key={v.r} className="rounded-xl border border-white/10 bg-white/5 p-3 mb-3">
            ...
            <div className="mt-2 flex gap-2">
              <Btn onClick={() => onNavigate?.(v.name)}>Navigate</Btn>
              <Btn glow onClick={() => onRallyHere?.(v.name)}>Rally Here</Btn>
              <Btn onClick={() => onHistory?.(v.name)}>History</Btn>
            </div>
          </div>
        ))}
      </Section>
