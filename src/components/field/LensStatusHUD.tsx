...
export function LensStatusHUD({ insight }: { insight?: string }) {
  ...
  return (
    <div className="fixed left-6 top-[calc(80px+env(safe-area-inset-top))] z-[560] px-3 py-2 rounded-md bg-black/35 backdrop-blur text-white/90 text-xs pointer-events-none">
      <b>{label}</b> • {insight}
    </div>
  )
}