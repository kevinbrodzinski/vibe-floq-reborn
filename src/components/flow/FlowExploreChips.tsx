...
export function FlowExploreChips({ value, onChange, clusterRes, loading, sunScore, inline = false, className }: Props) {
  ...
  const chipsContent = (
    <div
      ...
      className="flex items-center gap-2 min-w-fit bg-[color:var(--bg-alt)]/80 backdrop-blur-sm border border-[color:var(--border)] rounded-kit-lg px-3 py-2 pointer-events-auto select-none"
    ...
    >
      ...
      >
        Sun{typeof sunScore === 'number' ? ` · ${Math.round(sunScore*100)}%` : ''}
      </button>
  ...
  if (inline) {
    return (
      <div
        className={clsx(
          "pointer-events-auto overflow-x-auto touch-pan-x scrollbar-none max-w-full",
          className
        )}
      >
        {chipsContent}
  ...
  // legacy fixed path: keep scroll/taps working on the scroll container
  return (
    <div className="fixed left-0 right-0 top-after-topbar z-[660] pointer-events-none">
      <div className="mx-4 pointer-events-auto overflow-x-auto touch-pan-x scrollbar-none max-w-full">
        {chipsContent}
      </div>
    </div>
  )
