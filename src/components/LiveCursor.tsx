...
const CURSOR_FADE_TIME = 3000; // 3 s of inactivity
...
export const LiveCursor: React.FC<LiveCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const filtered = cursors.filter(
      (cursor) =>
        cursor.profileId !== currentUserId &&
  ...
  return (
    <div {...zIndex('system')} className={`fixed inset-0 pointer-events-none ${className}`}>
      <AnimatePresence>
        {activeCursors.map((cursor) => {
          ...
          return (
            <motion.div
              ...
              animate={{ scale: 1, opacity, x: cursor.x, y: cursor.y }}
            ...
            >
              ...
              <motion.div
                ...
                style={{ borderColor: cursor.color || '#3b82f6' }}
              />
            </motion.div>
          );
        })}
