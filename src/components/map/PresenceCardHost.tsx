...
/** Minimal inline card for converge acceptance */
...
export const PresenceCardHost: React.FC = () => {
  ...
  // Open presence card from legacy selection events (friend/venue)
  React.useEffect(() => {
  ...
  // Converge request receiver -> show minimal accept card
  React.useEffect(() => {
    if (typeof window === 'undefined') return;
    const onReq = (e: Event) => {
  ...
  // ESC to close any open lightweight cards
  React.useEffect(() => {
  ...
  return (
    <>
      ...
      <ConvergeSuggestions onClose={() => { /* modal self-manages */ }} />
      <AcceptRouteCard
        to={acceptTo}
        onAccept={() => {
          if (acceptTo) {
