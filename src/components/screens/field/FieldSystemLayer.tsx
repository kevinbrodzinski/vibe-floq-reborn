...
export const FieldSystemLayer = ({ data }: FieldSystemLayerProps) => {
  ...
  return (
    <>
      ...
      {process.env.NODE_ENV === 'development' && autoCheckIn.isEnabled && (
        <div 
          ...
          <div className="bg-accent text-accent-foreground px-3 py-2 rounded-lg shadow-lg text-sm">
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${
                autoCheckIn.isTracking ? 'bg-green-400 animate-pulse' : 'bg-gray-400'
              }`}></div>
              <div>
                <div className="font-medium">Auto Check-in Active</div>
                {autoCheckIn.currentVenue && (
                  <div className="text-xs opacity-90">
                    Tracking: {autoCheckIn.currentVenue}
                  </div>
                )}
              </div>
            </div>
