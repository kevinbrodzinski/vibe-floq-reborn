...
export function EnhancedPlanTimeline({
  ...
  const planMetrics = useMemo(() => {
    ...
    const activeParticipants = participants.filter(p => p.rsvp_status === 'accepted').length
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center gap-3">
                  ...
                  {participants.filter(p => p.rsvp_status === 'accepted').length > 6 && (
                    <Badge variant="secondary" className="text-xs">
                      +{participants.filter(p => p.rsvp_status === 'accepted').length - 6} more
                    </Badge>
                  )}
                </div>
              </CardContent>
