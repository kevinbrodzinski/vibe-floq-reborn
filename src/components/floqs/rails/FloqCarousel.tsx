...
export function FloqCarousel({ children }: { children: React.ReactNode[] }) {
  ...
  return (
    <div className="relative">
      <div ref={ref} className="flex snap-x snap-mandatory gap-4 overflow-x-auto px-2 py-2"
      ...
      {len > 1 && (
        <>
          <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-1">
            <Button size="icon" variant="secondary" className="pointer-events-auto h-8 w-8 rounded-full" onClick={prev}>‹</Button>
          </div>
          <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-1">
            <Button size="icon" variant="secondary" className="pointer-events-auto h-8 w-8 rounded-full" onClick={next}>›</Button>
          </div>
          <div className="mt-2 flex items-center justify-center gap-1">
            {Array.from({ length: len }).map((_, k) => (
              <button key={k} aria-label={`Go to slide ${k+1}`} onClick={()=>toIndex(k)}
