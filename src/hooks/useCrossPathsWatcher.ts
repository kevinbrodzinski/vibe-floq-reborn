...
type Options = { bestieRadiusM?: number; friendRadiusM?: number; cooldownMin?: number; throttleMs?: number };

const DEFAULTS: Required<Options> = { bestieRadiusM: 200, friendRadiusM: 400, cooldownMin: 45, throttleMs: 5000 };
...
export function useCrossPathsWatcher(
  ...
  useEffect(() => {
    ...
    for (const f of friends) {
      ...
      const d = haversineMeters({ lat: my.lat!, lng: my.lng! }, { lat: f.lat!, lng: f.lng! });
      const limit = f.tier === 'bestie' ? bestieRadiusM : f.tier === 'friend' ? friendRadiusM : 0;
      if (limit <= 0) continue;
