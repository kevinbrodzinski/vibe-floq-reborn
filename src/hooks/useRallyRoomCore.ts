...
/**
 * Core Rally Room functionality - NO PRIVACY GATES
 * This is the foundation that privacy can be added to optionally
 */
export function useRallyRoomCore(rallyId?: string) {
  ...
  return {
    ...
    async pingBasic(userId: string, etaBand?: string) {
      // Basic ping without privacy gates
      await broadcast({ 
        type: 'rally_ping', 
        rallyId: rallyId!, 
        userId, 
